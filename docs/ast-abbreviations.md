...
### 1. AST Node Type Prefixes

In esbuild's AST design, single-letter prefixes are used to distinguish different categories of node types:

| Prefix | Category    | Examples                                    |
| ------ | ----------- | ------------------------------------------- |
| `B`    | Binding     | `BIdentifier`, `BArray`, `BObject`          |
| `E`    | Expression  | `EIdentifier`, `EBinary`, `ECall`, `EArrow` |
| `S`    | Statement   | `SBlock`, `SIf`, `SFor`, `SExpr`            |
| `R`    | Rule        | `RAtImport`, `RSelector`, `RDeclaration`    |
| `D`    | Declaration | `DColor`, `DFlex`, `DUnknown`               |
| `MQ`   | Media Query | `MQType`, `MQBinary`, `MQNot`               |

### 2. Common Boolean Flag Prefixes

| Prefix   | Meaning                        | Examples                                      |
| -------- | ------------------------------ | --------------------------------------------- |
| `Has`    | Has a certain property/feature | `HasRestArg`, `HasNoSideEffectsComment`       |
| `Is`     | Is in a certain state          | `IsAsync`, `IsExport`, `IsSingleLine`         |
| `CanBe`  | Can be ...                     | `CanBeRemovedIfUnused`                        |
| `Was`    | Was ...                        | `WasOriginallyBareImport`                     |
| `Should` | Should ...                     | `ShouldLowerStandardDecorators`               |
| `Did`    | Already did ...                | `DidKeepName`                                 |
| `Use`    | Use ...                        | `UseCountEstimate`, `UseDefineForClassFields` |

### 3. Common Suffixes

| Suffix  | Meaning             | Examples                                         |
| ------- | ------------------- | ------------------------------------------------ |
| `OrNil` | May be nil/optional | `ValueOrNil`, `TestOrNil`, `TagOrNil`            |
| `Loc`   | Source location     | `CloseBraceLoc`, `NameLoc`, `BlockLoc`           |
| `Ref`   | Symbol reference    | `NamespaceRef`, `ArgumentsRef`, `WrapperRef`     |
| `Idx`   | Index               | `ImportRecordIndex`, `SourceIndex`, `InnerIndex` |
