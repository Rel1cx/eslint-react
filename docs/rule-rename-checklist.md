...
Step-by-step checklists for renaming or moving a rule in `eslint-plugin-react-x` and `eslint-plugin-react-simple`.
...
### A7. Preset Configs (`eslint-plugin-react-simple`)
...
### B5. Preset Configs (`eslint-plugin-react-simple`)
