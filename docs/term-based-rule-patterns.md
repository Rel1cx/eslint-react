...
### 2.2 Named-constant term check

Several `react-dom` rules hoist the target term into a module-level constant and
reuse it for both the precheck and the per-node comparison. This keeps the
literal in one place and avoids typos.
...
- `no-dangerously-set-innerhtml` (`DSIH`)
- `no-dangerously-set-innerhtml-with-children` (`DSIH`)
- `no-find-dom-node` (`findDOMNode`)
- `no-flush-sync` (`flushSync`)
- `no-hydrate` (`hydrate`)

### 2.3 Multi-term check
...
### 2.5 Non-identifier substring check
...
3. **Hoist the term into a constant when it is reused** for the per-node
   comparison (the `react-dom` pattern, §2.2). This prevents the precheck and
   the check from drifting apart.
...
5. **OR-combine prechecks when any one of several terms can trigger the rule**
   (§2.3). AND-combine them only when _all_ concepts must co-occur — for
