...
- [Custom Rules for Unsupported Rules](/docs/migrating-from-eslint-plugin-react#custom-rules-for-unsupported-rules)
  ...
  - [`no-unescaped-entities`](/docs/migrating-from-eslint-plugin-react#no-unescaped-entities)
...
| [`no-unescaped-entities`](https://github.com/jsx-eslint/eslint-plugin-react/blob/master/docs/rules/no-unescaped-entities.md)                                 | [`@eslint-react/kit/no-unescaped-entities`](#no-unescaped-entities); report-only                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | ✅           | ✅      |
...
## Custom Rules for Unsupported Rules

Some `eslint-plugin-react` rules marked as ❌ in the comparison table don't have built-in equivalents in ESLint React.
You can use [`@eslint-react/kit`](https://github.com/Rel1cx/eslint-react/blob/main/packages/utilities/kit/README.md) to create minimal custom implementations for these rules directly in your `eslint.config.ts`.
...
Below are drop-in rule definitions for the most commonly needed rules. Pass them to the `eslintReactKit()` call in your config:
...
export default defineConfig([
  {
    ...
    extends: [
      ...
      eslintReactKit(
        ...
        forbidComponentProps({ forbidden: ["className", "style"] }),
        forbidElements({
          forbidden: new Map([
...
/** Require `onChange` or `readOnly` when using `checked` on `<input>`. */
function checkedRequiresOnchangeOrReadonly(): RuleDefinition {
  return {
...
function forbidComponentProps({ forbidden }: ForbidComponentPropsOptions): RuleDefinition {
  return {
...
function jsxBooleanValue(): RuleDefinition {
  return {
...
function jsxNoBind(): RuleDefinition {
  return {
    name: "jsx-no-bind",
    make: (context) => ({
      JSXAttribute(node) {
...
/** Disallow JSX props spreading. */
function jsxPropsNoSpreading(): RuleDefinition {
  return {
...
/** Prevent defining more than one component per file. */
function noMultiComp(): RuleDefinition {
  return {
    name: "no-multi-comp",
    make: (context, { collect }) => {
      const { query, visitor } = collect.components(context);
...
Disallow unescaped `>`, `"`, `'`, and `}` in JSX text.
...
function noUnescapedEntities({ entities }: NoUnescapedEntitiesOptions): RuleDefinition {
