...
<Callout type="info">
  This guide provides a comprehensive comparison between [`eslint-plugin-react`](https://github.com/jsx-eslint/eslint-plugin-react) and ESLint React rules to help migrate existing configurations.
</Callout>
...
ESLint React is a collection of ESLint plugins and utilities for React, with a focus on performance, TypeScript integration, and alignment with [react.dev](https://react.dev) guidelines.
It offers a different set of rules and design choices compared to `eslint-plugin-react`.
...
- ✅ **Mostly supported** - Rule is supported but does not have an auto-fix
...
- 🚫 **Legacy** - Rule is not applicable in modern TypeScript React development (for example, class-based components, `propTypes`)
...
- 🧰 **Supported via [`@eslint-react/kit`](/docs/packages/kit)** - Rule is supported as a custom rule via [`@eslint-react/kit`](/docs/packages/kit)

A variety of rules are marked **legacy**, but still have equivalent rules.
This distinction was done to more accurately assess migration for React written with function components and no longer using `propTypes`.
...
| :----------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------- | :----- |
| [`boolean-prop-naming`](https://github.com/jsx-eslint/eslint-plugin-react/blob/master/docs/rules/boolean-prop-naming.md)                                     | [`@eslint-react/kit/boolean-prop-naming`](#boolean-prop-naming) report-only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | ✅           | 💭     |
...
| [`destructuring-assignment`](https://github.com/jsx-eslint/eslint-plugin-react/blob/master/docs/rules/destructuring-assignment.md)                           | [`@eslint-react/kit`](#destructuring-assignment) (custom rule)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 🔧          | 🧰     |
...
| [`function-component-definition`](https://github.com/jsx-eslint/eslint-plugin-react/blob/master/docs/rules/function-component-definition.md)                 | [`@eslint-react/kit`](https://github.com/Rel1cx/eslint-react/blob/main/packages/kit/README.md) (custom rule, for example [`@eslint-react/kit/function-component-definition`](#function-component-definition); report-only by default, autofix depends on the implementation)                                                                                                                                                                                                                                                                                   | ✅           | 🔧     |
...
| [`no-invalid-html-attribute`](https://github.com/jsx-eslint/eslint-plugin-react/blob/master/docs/rules/no-invalid-html-attribute.md)                         | _See also [`dom-no-unknown-property`](/docs/rules/dom-no-unknown-property)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | ✅           | ❌      |
...
| [`no-set-state`](https://github.com/jsx-eslint/eslint-plugin-react/blob/master/docs/rules/no-set-state.md)                                                   | [`@eslint-react/kit/no-set-state`](#no-set-state) report-only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ✅           | 🧰     |
| [`no-string-refs`](https://github.com/jsx-eslint/eslint-plugin-react/blob/master/docs/rules/no-string-refs.md)                                               | [`@eslint-react/kit/no-string-refs`](#no-string-refs) report-only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | ✅           | 🧰     |
...
Migrate gradually by using both plugins together with the `disable-conflict-eslint-plugin-react` ruleset:
...
export default defineConfig([
  // Start with the eslint-plugin-react
  {
    ...
    extends: [
      // Whatever config was enabled with eslint-plugin-react
...
After full migration, remove `eslint-plugin-react` entirely and rely solely on ESLint React:
...
Some `eslint-plugin-react` rules do not have built-in equivalents in ESLint React.
Use [`@eslint-react/kit`](/docs/packages/kit) to create minimal custom rule implementations directly in the `eslint.config.ts`.
...
Below are drop-in rule definitions for the most commonly needed rules. Register them via the `.use()` chain in the config:
...
export default defineConfig([
  {
    ...
    extends: [
      eslintReact.configs["recommended-typescript"],
      eslintReactKit()
        .use(booleanPropNaming, { rule: "^(is|has|should)[A-Z]([A-Za-z0-9]?)+" })
...
```ts twoslash title=".config/booleanPropNaming.ts"
...
Forbid specific JSX elements. Customize the `forbidden` map to match project requirements.
...
Enforce shorthand for boolean JSX attributes (for example, prefer `<C disabled />` over `<C disabled={true} />`).
...
Reports when `<React.Fragment>` is used instead of `<>...</>`.
Allows standard form when `key` prop is present.
  ...
  return (context, { ast }) => ({
    CallExpression(node) {
      ...
      if (
        ...
        context.report({
          node,
          message: "this.setState() is not allowed.",
        });
      }
    },
  });
  ...
  return (context) => ({
    JSXAttribute(node) {
      ...
      if (value?.type === "Literal" && typeof value.value === "string") {
        context.report({
          node,
          message: `String refs are deprecated and should not be used. Use callback refs or React.createRef() instead.`,
        });
      }
    },
  });
