...
description: How to configure ESLint React analyzer settings
---
...
This allows rules to align their behavior with the React Compiler's optimization strategies. Set this to the same value as your React Compiler config (ex: `"infer"`, `"annotation"`, `"syntax"`, or `"all"`).
...
A regex pattern for matching custom hooks that should be treated as ref hooks. The value must be a valid JavaScript RegExp string with delimiters and flags.

Example matching `useMyRef` and `useCustomRef`:
...
A regex pattern for matching custom hooks that should be treated as effect hooks. The value must be a valid JavaScript RegExp string with delimiters and flags.

Example matching `useMyEffect` and `useCustomEffect`:
...
A regex pattern for matching custom hooks that return external mutable stores (like the values returned by navigation hooks). Writes through such values are not reported as mutations of captured local values. The value must be a valid JavaScript RegExp string with delimiters and flags.

Example matching `useWorld` and `useMyStore`:
...
export default [
  {
    ...
    settings: {
      "react-x": {
        ...
        additionalRefHooks: "/^(useMyRef|useCustomRef)$/u", // Regex pattern for custom hooks treated as ref hooks
        additionalStateHooks: "/^(useMyState|useCustomState)$/u", // Regex pattern for custom hooks treated as state hooks
        additionalEffectHooks: "/^(useMyEffect|useCustomEffect)$/u", // Regex pattern for custom hooks treated as effect hooks
        additionalMutableHooks: "/^(useWorld|useMyStore)$/u", // Regex pattern for custom hooks returning external mutable stores
      },
    },
  },
];
