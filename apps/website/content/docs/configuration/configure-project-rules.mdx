...
<Callout type="info">
  This utility module is for advanced use cases where custom ESLint rules are built to match the project's specific requirements.
  For full documentation on the kit module, including API reference and examples, see the [Kit documentation](/docs/packages/kit).
</Callout>
...
Define custom rules directly in the `eslint.config.ts`:
