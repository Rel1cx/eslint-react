...
- **`disable-conflict-eslint-plugin-react-hooks` configuration**: Added to both `eslint-plugin-react-xtended` and `eslint-plugin-react-x` for easier migration from `eslint-plugin-react-hooks`.
