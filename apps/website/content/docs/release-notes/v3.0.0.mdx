...
- **`disable-conflict-eslint-plugin-react-hooks` configuration**: Added to both `eslint-plugin-react-simple` and `eslint-plugin-react-x` for easier migration from `eslint-plugin-react-hooks`.
