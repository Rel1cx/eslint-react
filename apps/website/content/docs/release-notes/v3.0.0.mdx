...
This release adds a significant number of new rules to `eslint-plugin-react-x`, including rules ported _verbatim_ from `eslint-plugin-react-hooks` with full code-path analysis, as well as re-implemented rules from `eslint-plugin-react-compiler` that offer better performance and, unlike the official plugin, require **no** Babel or React Compiler installation:
