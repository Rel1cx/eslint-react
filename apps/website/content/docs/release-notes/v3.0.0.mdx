...
- **`react-x/rules-of-hooks`**: Enforces the [Rules of Hooks](https://react.dev/reference/rules/rules-of-hooks). Ported from `eslint-plugin-react-hooks` with code-path analysis for more accurate hook validation.
...
- Remove `eslint-plugin-react-hooks-extra` from `package.json` — this package is no longer published.
- If `eslint-plugin-react-hooks-extra` was imported directly, replace it with `eslint-plugin-react-x`.
...
4. Remove the following removed/consolidated rules from the config if present:
...
- If the project uses [React Compiler](https://react.dev/learn/react-compiler), consider adding the `compilationMode` setting under `settings["react-x"]`.

### Review New Rules Enabled in Presets

When using the `recommended`, `x`, or `all` preset, the following rules are now included automatically:
