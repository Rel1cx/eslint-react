...
1. **`noDirectAccessProps`**: Reports direct member expression access of component props (e.g., `props.name`) and enforces destructuring assignment instead.
2. **`noDuplicateProps`**: Reports duplicate props on a JSX element.
