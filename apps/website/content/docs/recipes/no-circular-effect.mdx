...
<Callout type="warning">
  For most cases, prefer the built-in [`set-state-in-effect`](/docs/rules/set-state-in-effect) rule, which is now also provided in [React's official lints](https://react.dev/reference/eslint-plugin-react-hooks/lints/set-state-in-effect), to better follow the guidance from the [official documentation](https://react.dev/learn/you-might-not-need-an-effect). Use this custom recipe only when you need to detect circular dependencies across multiple effects.
</Callout>
