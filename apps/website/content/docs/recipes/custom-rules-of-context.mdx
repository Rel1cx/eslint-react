...
Reports when `useContext` is used in React 19+ projects. The `use` API is more flexible than `useContext` because it can be called in loops and conditional statements.

This rule checks the React version from settings and only reports when the version is 19.0.0 or higher. For projects on React 18 or earlier, this rule does nothing.
...
- [`custom-rules-of-children`](./custom-rules-of-children)\
  Validates React Children API usage. Includes checks for `Children.count`, `Children.forEach`, `Children.map`, `Children.only`, and `Children.toArray`.
