...
Recipes are ready-to-use custom ESLint rule definitions built with [`@eslint-react/kit`](/docs/packages/kit).
They are **not** part of the core plugin.
Instead, they are designed to be **copied and pasted** directly into the project.
Each recipe contains one or more custom rules.

## Why Recipes?

Some lint rules are inherently **opinionated**.
They enforce a specific code style or convention that not every team agrees on.
Rather than shipping these as built-in rules with fixed behavior, they are provided as recipes so that teams can:

- **Copy** the rule definition into the project.
- **Customize** the implementation to match team conventions.
- **Own** the code with no dependency on unrequested upstream changes.
...
1. **Rule**: A self-contained function to copy into the project (for example into a `.config/noForwardRef.ts` file).
...
| [`No Circular Effect`](/docs/recipes/no-circular-effect)                       | Detects circular dependencies between useEffect hooks.
Prevents infinite update loops caused by effects that set state they also depend on. |
| [`No Multiple Children in Title`](/docs/recipes/no-multiple-children-in-title) | Ensures the `<title>` component has a single string child.
This avoids React errors.                                                            |
