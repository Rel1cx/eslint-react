...
2. **Config**: How to wire it up in the `eslint.config.ts` using `eslintReactKit().use(...)`.
...
| [`No Multiple Children in Title`](/docs/recipes/no-multiple-children-in-title) | Ensures the `<title>` component has a single string child. This avoids React errors.                                                        |
