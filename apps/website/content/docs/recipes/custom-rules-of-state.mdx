...
1. **[`preferStateUpdaterFunction`](#preferstateupdaterfunction)**: Reports when a `useState` setter is called with an expression that directly references the corresponding state variable.
   Using the callback (updater function) form ensures the update uses the latest state value, avoiding stale-state bugs in closures, event handlers, and timeouts.

## preferStateUpdaterFunction

Copy the following code into the project (for example `.config/preferStateUpdaterFunction.ts`):
...
### Referencing state directly in the setter
...
function Counter() {
  ...
  // Problem: Referencing `count` directly creates a stale state risk.
  return (
...
function Toggle() {
  ...
  // Problem: Negating state directly creates a stale state risk.
  return (
...
function UserEditor() {
  ...
  // Problem: Spreading state directly creates a stale state risk.
...
function ItemList() {
  ...
  // Problem: Calling a method on state directly creates a stale state risk.
...
### Using the updater function or a constant value
...
function Counter() {
  ...
  // Recommended: Callback form always gets the latest state.
  return (
...
function Component() {
  ...
  // OK: Setting a constant value does not reference state.
...
function Component() {
  ...
  // OK: Setting a value that does not reference the state variable.
...
function Component() {
  ...
  // OK: Referencing a *different* state variable is fine.
  setCount(total);
...
- [`custom-rules-of-props`](./custom-rules-of-props)\
  Custom rules for validating JSX props.
  Includes checks for duplicate props and mixing controlled and uncontrolled props.
