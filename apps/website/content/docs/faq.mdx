...
<Accordions>
  <Accordion title="Why?">
    **ESLint React addresses critical gaps in the existing `eslint-plugin-react` in modern React ecosystems**. While named "react", the current plugin implementation specifically targets React DOM and maintains DOM-centric assumptions, creating friction when used with alternative renderers like React Native, React Three Fiber, or custom renderers.
  ...
  <Accordion title="What's included?">
    ...
    **Public Packages**
    ...
    - **Utilities**
      - `packages/utilities/kit`: Utility module for building custom React rules with JavaScript functions
    - **ESLint Plugins**
      - [`eslint-plugin-react-x`](https://github.com/Rel1cx/eslint-react/tree/stable/packages/plugins/eslint-plugin-react-x) - Core React rules.
      - [`eslint-plugin-react-jsx`](https://github.com/Rel1cx/eslint-react/tree/stable/packages/plugins/eslint-plugin-react-jsx) - React Flavored JSX rules.
      - [`eslint-plugin-react-rsc`](https://github.com/Rel1cx/eslint-react/tree/stable/packages/plugins/eslint-plugin-react-rsc) - React Server Components rules.
      - [`eslint-plugin-react-dom`](https://github.com/Rel1cx/eslint-react/tree/stable/packages/plugins/eslint-plugin-react-dom) - React DOM rules.
      - [`eslint-plugin-react-web-api`](https://github.com/Rel1cx/eslint-react/tree/stable/packages/plugins/eslint-plugin-react-web-api) - Web API interaction rules.
      - [`eslint-plugin-react-naming-convention`](https://github.com/Rel1cx/eslint-react/tree/stable/packages/plugins/eslint-plugin-react-naming-convention) - Naming convention rules.
      - [`eslint-plugin-react-debug`](https://github.com/Rel1cx/eslint-react/tree/stable/packages/plugins/eslint-plugin-react-debug) - Debugging rules for inspecting React patterns in code.
      - [`@eslint-react/eslint-plugin`](https://github.com/Rel1cx/eslint-react/tree/stable/packages/plugins/eslint-plugin) - A unified plugin that combines all individual plugins into one.
