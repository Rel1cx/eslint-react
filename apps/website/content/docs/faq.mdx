...
- **Configure Analyzer Settings**: Customize static analysis behavior via the `react-x` key in your ESLint `settings` object:
  ...
  - **Custom Hooks**: Use `additionalRefHooks`, `additionalStateHooks` and `additionalEffectHooks` regex patterns to help the analyzer recognize and validate your custom hooks as standard ref, state or effect hooks.
  - **External Mutable Stores**: Use the `additionalMutableHooks` regex pattern for custom hooks that return external mutable stores, so writes through their return values are not reported as mutations of captured local values.
