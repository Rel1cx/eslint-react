...
| [`no-direct-set-state-in-use-effect`](./no-direct-set-state-in-use-effect)               | 1️⃣  | `🧪`      | Disallow direct calls to the `set` function of `useState` in `useEffect`                             |          |
| [`no-direct-set-state-in-use-layout-effect`](./no-direct-set-state-in-use-layout-effect) | 0️⃣  | `🧪`      | Disallow direct calls to the `set` function of `useState` in `useLayoutEffect`                       |          |
