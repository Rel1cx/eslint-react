...
| `useRefLikeCall`           | `(node, additionalHooks?) -> boolean` | Whether a node is a `useRef`-like call.                      |
...
<Accordions>
  <Accordion title="Why `is.API` / `is.APICall` instead of a hand-written check?">
    ...
    ```ts
    const ownerStack = ((React as any).captureOwnerStack)?.()! satisfies OwnerStack;
    ```

    The call to `captureOwnerStack` is nested within four layers of syntax:

    1. a `React as any` cast (`TSAsExpression`),
    2. an optional call (`?.()` → `ChainExpression`),
    3. a trailing non-null assertion (`!` → `TSNonNullExpression`), and
    4. a `satisfies OwnerStack` clause (`TSSatisfiesExpression`).

    A naive guard such as `node.callee.type === "Identifier" && node.callee.name === "captureOwnerStack"` matches **nothing**: the callee is not an `Identifier` — it is a `MemberExpression` whose `object` is a `TSAsExpression`.
    ...
    Use `is.API(name)` for the reference itself and `is.APICall(name)` for a call to it. Hand-rolling this means re-implementing unwrapping and member-expression resolution for **every** API — and breaking the moment someone adds a `?.`, an `as`, or a `React.` prefix.
  </Accordion>
</Accordions>
...
| ----------------------- | ------------------------------------------------------- | ----------- | --------------------------------------------------------- |
| `version`               | `string`                                                | auto-detect | Resolved React version (e.g. `"19.3.0"`).                 |
...
| `polymorphicPropName`   | `string`                                                | `"as"`      | Prop name used for polymorphic components.                |
| `additionalRefHooks`    | `RegExpLike`                                            | —           | Pattern matching custom hooks treated as ref hooks.       |
...
<Accordions>
  <Accordion title="Simple: Ban `forwardRef`">
  ...
  <Accordion title="Component: Destructure component props">
  ...
  <Accordion title="Hooks: Warn on custom hooks that don't call other hooks">
  ...
  <Accordion title="Multiple Collectors: Error Boundaries">
  ...
  <Accordion title="Override Config: Using spread syntax with `getConfig`">
  ...
  <Accordion title="Advanced Config: Using `getPlugin` for custom plugin namespace">
