...
When an **anonymous function** (arrow function without a name) is used with `.use()`, a random hex string is automatically generated as the rule name:
...
Anonymous rules are ideal for checks that are **critical to code quality or security** and must never be bypassed via disable comments:

```ts
// This critical security check cannot be disabled in source code
eslintReactKit().use(() => (context) => ({
  Property(node) {
...
**Note:** Since the rule name is random and changes on every ESLint run, standard rules configs and disable comments do not work:
...
To disable an anonymous rule, modify the ESLint configuration file directly. This provides an audit trail for policy violations.
...
Returns an `ESLint.Plugin` object containing the registered rules and plugin metadata (`name` and `version`). Use this for finer-grained control over how the plugin is integrated into the ESLint configuration.
...
// Use it in a custom flat config with a custom namespace and severity
export default [
  {
    files: ["**/*.{ts,tsx}"],
    plugins: {
...
This is essential for combining a collector's `visitor` with custom inspection logic.
...
Collector factories create a `{ query, visitor }` pair. The `visitor` must be merged into the rule listener via `merge()`. After traversal completes, `query.all(program)` yields all detected semantic nodes.
...
All React API predicates and factories have `context` pre-bound, eliminating the need to pass the rule context manually:
...
`ast.unwrap` is useful when inspecting a callee, argument, or initializer that might be wrapped in a TypeScript type assertion:
...
Exposes the normalized `react-x` settings from the ESLint shared configuration (`context.settings["react-x"]`). This lets custom rules read and react to the same project-level settings used by the built-in rules.
...
```ts twoslash
import eslintReactKit, { type RuleFunction, merge } from "@eslint-react/kit";
...
`getConfig()` returns a plain config object. Spread it into a new object to override or supplement its properties — for example, to scope the config to specific files or add extra settings alongside kit rules.
...
export default [
  {
    ...eslintReactKit()
      ...
      .use(version, "19")
      .getConfig(),
    // Override `name` so it shows a custom label in config inspector tools
...
This pattern is especially useful when composing kit configs alongside other ESLint configs (e.g. `typescript-eslint`, `eslint-plugin-react-hooks`) via `defineConfig` — nest the spread object inside `extends` arrays and attach shared properties like `files` or `languageOptions` at the same level.

### Advanced Config: Using `getPlugin` for custom plugin namespace

Use `getPlugin()` for full control over the plugin namespace and rule severities instead of the all-in-one `getConfig()`.
...
<AutoClick selector={`button[aria-label="Close Banner"]`} />
