...
function MyComponent() {
  // ✅ It is appropriate to use `useRef` here because the ref is used outside the effect as well.
...
function MyComponent() {
  // ✅ This ref can be used for other purposes, and if not, the no-unused-vars rule will catch it.
  const ref = useRef<number | null>(null);
...
- [Rule Source](https://github.com/Rel1cx/eslint-react/tree/main/packages/plugins/eslint-plugin-react-x/src/rules-removed/no-unnecessary-use-ref.ts)
- [Test Source](https://github.com/Rel1cx/eslint-react/tree/main/packages/plugins/eslint-plugin-react-x/src/rules-removed/no-unnecessary-use-ref.spec.ts)
