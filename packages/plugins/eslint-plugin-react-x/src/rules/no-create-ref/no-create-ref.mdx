---
title: no-create-ref
---

**Full Name in [`@eslint-react/eslint-plugin`](https://npmx.dev/package/@eslint-react/eslint-plugin/v/latest)**

```plain copy
@eslint-react/no-create-ref
```

**Full Name in [`eslint-plugin-react-x`](https://npmx.dev/package/eslint-plugin-react-x/v/latest)**

```plain copy
react-x/no-create-ref
```

**Presets**

`x`
`recommended`
`recommended-typescript`
`recommended-type-checked`
`strict`
`strict-typescript`
`strict-type-checked`

## Rule Description

Disallows `createRef` in function components.

`createRef()` is a legacy API that is not recommended for use in new code. Instead, prefer using `useRef()` with function components.

## Triggering Examples

```tsx
import React, { createRef } from "react";

function MyComponent() {
  const ref = React.createRef<HTMLDivElement>();
  //          ^^^ [Deprecated] Use 'useRef' instead.

  return <div ref={ref} />;
}
```

## Non Triggering Examples

```tsx
import React, { useRef } from "react";

function MyComponent() {
  const ref = useRef<HTMLDivElement>(null);

  return <div ref={ref} />;
}
```

```tsx
import React, { createRef } from "react";

class MyComponent extends React.Component {
  inputRef = createRef();
  // ...
}
```

## Resources

- [Rule Source](https://github.com/Rel1cx/eslint-react/tree/main/packages/plugins/eslint-plugin-react-x/src/rules/no-create-ref/no-create-ref.ts)
- [Test Source](https://github.com/Rel1cx/eslint-react/tree/main/packages/plugins/eslint-plugin-react-x/src/rules/no-create-ref/no-create-ref.spec.ts)

## Further Reading

- [Legacy React APIs: `createRef`](https://react.dev/reference/react/createRef)

---

## See Also

- [`no-forward-ref`](./no-forward-ref)\
  Replaces usage of  with passing  as a prop
- [`refs`](./refs)\
  Validates correct usage of refs by checking that  is not read or written during render
- [`naming-convention/ref-name`](./naming-convention-ref-name)\
  Enforces identifier names assigned from  calls to be either  or end with