...
If a ref is only used inside a single effect, the ref can be replaced by a local variable scoped to that effect. This avoids persisting mutable state across renders when it is only needed within one effect’s lifetime.
