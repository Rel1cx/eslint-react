...
function MyComponent() {
  ...
  //    - Unnecessary use of `useRef`. Instead, co-locate the value inside the effect that uses it.
...
function MyComponent() {
  useEffect(() => {
    ...
    //  🟢 Using a local variable to co-locate the value inside the effect that uses it.
    return () => {
      if (ref != null) {
