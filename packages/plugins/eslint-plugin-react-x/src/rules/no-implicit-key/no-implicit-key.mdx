...
1. The `key` property originates from React's own type definitions (e.g. `React.Attributes.key`), such as when spreading `React.ComponentProps<"div">`, `React.HTMLAttributes<T>`, or similar React-provided types.
