...
export function create(context: RuleContext<MessageID, []>): RuleListener {
  ...
  return {
    JSXAttribute(node: TSESTree.JSXAttribute) {
      ...
      if (jsxElement.openingElement.attributes.some((attr) => attr.type === AST.JSXSpreadAttribute)) return;
...
function isArrayMethodCallback(node: TSESTree.Node) {
  ...
  if (parent.callee.type !== AST.MemberExpression || parent.callee.property.type !== AST.Identifier) return false;
