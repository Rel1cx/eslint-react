...
const updateDocumentation = Effect.fnUntraced(
  function*(plugin: PluginDomain, oldName: string, newName: string) {
    ...
    // Update full name in eslint-plugin-react-instant code block
...
const updateAggregatedConfigs = Effect.fnUntraced(
  function*(plugin: PluginDomain, oldName: string, newName: string) {
    ...
    const configGlob = ["packages/eslint-plugin-react-instant/src/configs/*.ts"];
...
const verifyNoLeftovers = Effect.fnUntraced(
  function*(plugin: PluginDomain, oldName: string) {
    const pluginDir = `packages/eslint-plugin-react-${plugin}`;
    const aggregatedDir = "packages/eslint-plugin-react-instant";
