...
export const getCommentAfter = (
    ...
    const token = source.getTokenAfter(node, {
        filter: ({ type, value }) => !(type === TSEUtils.AST_TOKEN_TYPES.Punctuator && [",", ";"].includes(value)),
        includeComments: true,
    });

    if (
        (token?.type === TSEUtils.AST_TOKEN_TYPES.Block || token?.type === TSEUtils.AST_TOKEN_TYPES.Line) &&
