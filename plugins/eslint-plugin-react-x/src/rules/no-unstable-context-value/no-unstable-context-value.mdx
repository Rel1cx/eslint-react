...
### Stabilizing context value with `useMemo`

When a context value depends on state or props, wrap it in `useMemo` so the reference stays stable across renders. This prevents unnecessary re-renders in all consumers.

```tsx
// Recommended: use useMemo to keep the context value reference stable
...
- [Rule Source](https://github.com/Rel1cx/eslint-react/tree/main/plugins/eslint-plugin-react-x/src/rules/no-unstable-context-value/no-unstable-context-value.ts)
- [Test Source](https://github.com/Rel1cx/eslint-react/tree/main/plugins/eslint-plugin-react-x/src/rules/no-unstable-context-value/no-unstable-context-value.spec.ts)
- [Rule Changelog](https://github.com/Rel1cx/eslint-react/tree/main/plugins/eslint-plugin-react-x/src/rules/no-unstable-context-value/CHANGELOG.md)
