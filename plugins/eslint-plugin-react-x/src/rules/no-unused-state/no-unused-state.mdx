...
This rule warns about state variables declared via `useState` (or similar state hooks) that are assigned but never read, or only read inside an effect or effect dependency array. State that is only used in effects often indicates missing logic in the render output or a state value that can be derived from props instead.
