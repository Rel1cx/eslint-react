...
// Local adaptations:
//   - `getAdditionalEffectHooksFromSettings` replaced by `context.settings`
//     from the `RichContext` provided by `@eslint-react/core`, reading
//     `additionalEffectHooks` from the `react-x` settings namespace.
//
...
const rule = {
  ...
  create(rawContext: Rule.RuleContext) {
    const context = buildRichContext(rawContext);
    const rawOptions = context._.options && context._.options[0];
    ...
    const additionalHooks = rawOptions && rawOptions.additionalHooks
      ...
      : context.settings.additionalEffectHooks;

    const enableDangerousAutofixThisMayCauseInfiniteLoops: boolean = (rawOptions
     ...
     */
    const getSourceCode = typeof context._.getSourceCode === "function"
      ? () => {
        return context._.getSourceCode();
      }
      : () => {
        return context.src;
      };
     ...
     */
    const getScope = typeof context._.getScope === "function"
      ? () => {
        return context._.getScope();
      }
      : (node: Node) => {
        return context.src.getScope(node);
      };
      ...
      function isInsideEffectCleanup(reference: Scope.Reference): boolean {
        ...
        while (curScope != null && curScope.block !== node) {
          if (curScope.type === "function") {
            isInReturnedFunction = curScope.block.parent != null
