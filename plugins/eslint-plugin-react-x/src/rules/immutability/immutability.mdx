...
Writes through values returned by navigation hooks (`useHistory`, `useNavigate`, `useNavigation`, `useRouter`) are exempted as well, since those values are external mutable stores rather than captured local values. Custom hooks that return such stores can be registered with the `additionalMutableHooks` setting.
...
Custom hooks that return external mutable stores (like the values returned by navigation hooks) can be registered with `additionalMutableHooks`:

```json
{
  "settings": {
    "react-x": {
      "additionalMutableHooks": "(useWorld|useMyStore)"
    }
  }
}
```
...
```tsx
// 🟢 Allowed: `world` is an external mutable store, not a captured local value
function Component() {
