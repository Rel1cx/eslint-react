...
Once something is marked as a ref, the lint surfaces violations when `ref.current` is accessed directly in the render phase of a component or hook body (i.e. not inside effects, event handlers, callbacks, or other nested functions). A nested function only shields the access it contains if that function is never actually invoked during render: calling a locally-defined helper (even through a simple alias) while rendering is still flagged, because the ref access effectively still happens during render.
