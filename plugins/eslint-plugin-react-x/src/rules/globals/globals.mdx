...
### Mutations through aliases and render helpers

Renaming a global value locally does not make it local. The rule also follows directly called helpers, because their side effects still run during render.
...
function Component() {
  const alias = cache;
  const updateCache = () => {
    alias.value = computeValue(); // Global mutation through an alias
  };
  updateCache(); // The helper runs during render
  return <div>{cache.value}</div>;
}
```

Helpers that run later in an effect or event handler are not treated as render-time mutations.
