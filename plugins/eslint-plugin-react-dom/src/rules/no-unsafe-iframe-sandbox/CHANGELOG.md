...
- Fixed a false negative where member-expression components (e.g. `<motion.div as="iframe" sandbox="allow-scripts allow-same-origin">`) were mistaken for host elements and the polymorphic prop was ignored; they are now resolved through the polymorphic prop and checked like the underlying DOM element.
- String values of the polymorphic prop are now normalized to lowercase, so `as="IFRAME"` is treated as `iframe`.
