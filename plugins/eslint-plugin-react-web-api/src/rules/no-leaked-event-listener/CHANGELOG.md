...
- Fixed a false negative where a computed identifier key in the listener options (e.g. `addEventListener("resize", handler, { [signal]: controller.signal })`) was treated as a static `signal`/`capture` option, masking leaked event listeners. The actual property name is the runtime value of the variable, so it is no longer recognized; computed string literal keys (e.g. `{ ["signal"]: controller.signal }`) are still recognized.
