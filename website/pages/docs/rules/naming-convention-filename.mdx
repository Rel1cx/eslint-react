...
- `excepts`: List of RegExp strings that should be ignored by this rule. The default value is `["^index$"]` which ignores files named `index`.
- `extensions`: List of file extensions that should be checked by this rule. The default value is `[".jsx", ".tsx"]` which checks only `.jsx` and `.tsx` files.
...
export default [
  // ...
  {
    files: ["**/*.tsx"],
    rules: {
      "@eslint-react/naming-convention/filename": ["warn", { "rule": "kebab-case", "excepts": ["^index$"] }]
  }
];
```
