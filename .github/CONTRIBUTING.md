...
Before submitting your contribution, please make sure to read through the [Code of Conduct](./CODE_OF_CONDUCT.md), as well as the appropriate section for the contribution you intend to make:
...
│   └── samples/                #   Sample fixtures and configs
├── apps/
│   ├── playground/             # Development playground
│   └── website/                # Documentation website
├── examples/                   # Example projects (Next.js, Preact, React DOM, etc.)
├── packages/
│   ├── ast/                    # @eslint-react/ast — AST utilities
│   ├── core/                   # @eslint-react/core — core analysis logic
│   ├── eslint/                 # @eslint-react/eslint — ESLint utilities
│   ├── jsx/                    # @eslint-react/jsx — JSX utilities
│   ├── kit/                    # @eslint-react/kit — shared kit utilities
...
```bash
# Full build (update metadata + build internal packages + build all packages + build plugins)
pnpm run build
...
# Run all linting checks (publish, TypeScript, ESLint, examples, text)
pnpm run lint
```
...
```
plugins/<plugin>/src/rules/<rule-name>/
...
3. Update the unified plugin (`plugins/eslint-plugin/`) to include the new rule.
...
1. Review and test all AI-generated content before submitting. You're the one responsible for it, not the AI.
